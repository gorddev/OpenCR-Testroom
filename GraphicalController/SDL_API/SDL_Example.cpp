...
GLuint create_GL_program(const std::filesystem::path& vertexShader, const std::filesystem::path& fragmentShader);

int main() {

    gan::Window winSDL = gan::Window::make("My SDL Context", {340, 300}, gan::WindowFloatOnTop);
    winSDL.setPosition({200, 300});
    int x, y;

    SDL_CreateRenderer(winSDL, "software");
    ...
    SDL_GetWindowSizeInPixels(winGL, &x, &y);
    glViewport(0, 0, x, y);
    ...
    GLuint vao = create_GL_vertex_array(vertices, 6);
    GLuint program = create_GL_program("../vshader.vert", "../fshader.frag");
